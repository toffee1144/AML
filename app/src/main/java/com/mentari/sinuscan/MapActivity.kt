...
import com.mentari.sinuscan.model.Hospital
import com.mentari.sinuscan.model.HospitalAdapter
...
class MapActivity : AppCompatActivity() {
        ...
        btnOpenMap.setOnClickListener {
            if (userLat != null && userLon != null) {
                ...
                if (mapIntent.resolveActivity(packageManager) != null) {
                    ...
                    Log.w("MapActivity", getString(R.string.map_fallback_browser))
                }
            } else {
                Log.w("MapActivity", getString(R.string.location_not_available))
            }
        }
    ...
    private fun getUserLocation() {
        ...
        fusedLocationClient.lastLocation.addOnSuccessListener { location ->
            if (location != null) {
                Log.d("MapActivity", getString(R.string.location_got, location.latitude, location.longitude))
                userLat = location.latitude
                userLon = location.longitude
                getAddress(location.latitude, location.longitude)
                getNearbyHospitals(location.latitude, location.longitude)
            } else {
                Log.e("MapActivity", getString(R.string.location_null))
            }
        }
    ...
    private fun getAddress(lat: Double, lon: Double) {
        ...
        client.newCall(request).enqueue(object : Callback {
            ...
            override fun onFailure(call: Call, e: IOException) {
                ...
                runOnUiThread { tvLocation.text = getString(R.string.location_failed) }
            }
        })
    }

    private fun getNearbyHospitals(lat: Double, lon: Double) {
        val radius = 10000  // 5 km
        ...
        client.newCall(request).enqueue(object : Callback {
            ...
            override fun onFailure(call: Call, e: IOException) {
                ...
                Log.e("MapActivity", getString(R.string.hospital_fetch_failed, e.message))
